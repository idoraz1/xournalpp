#include "XYScatterGraphHandler.h"
...
XYScatterGraphHandler::XYScatterGraphHandler(Control* control, const PageRef& page, bool flipShift,
                                             bool flipControl):
        BaseShapeHandler(control, page, flipShift, flipControl) {}

XYScatterGraphHandler::~XYScatterGraphHandler() = default;

auto XYScatterGraphHandler::createShape(bool isAltDown, bool isShiftDown, bool isControlDown)
    ...
    Range rg(p1.x - std::abs(width), p1.y - std::abs(height));
    ...
    // Vertical line (disconnected)
    pts.emplace_back(p1.x, p1.y - std::abs(height), Point::NO_PRESSURE);
